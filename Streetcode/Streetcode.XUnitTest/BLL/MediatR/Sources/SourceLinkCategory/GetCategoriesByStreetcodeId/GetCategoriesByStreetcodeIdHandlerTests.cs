...
using Streetcode.DAL.Specifications.Base; // Важливо!
using Xunit;
...
public class GetCategoriesByStreetcodeIdHandlerTests
{
    ...
    public async Task Handle_ShouldReturnFail_WhenCategoriesAreNull()
    {
        ...
        // Пофіксений мок під специфікації
        _repositoryWrapperMock
            .Setup(x => x.SourceCategoryRepository.GetAllAsync(It.IsAny<ISpecification<SourceLinkCategoryEntity>>()))
        ...
        _loggerMock.Verify(
            logger => logger.LogError(query, "Cant find any source category with the streetcode id 1"),
            Times.Once);
    }
